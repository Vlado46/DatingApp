...
public class LikesRepository(AppDbContext context) : ILikesRepository
{
    ...
    public async Task<PaginatedResult<Member>> GetMemberLikes(LikesParams likesParams)
    {
        ...
        {
            case "liked":
                result = query.Where(l => l.SourceMemberId == likesParams.MemberId).Select(l => l.TargetMember);
                break;
            case "likedBy":
                result = query.Where(l => l.TargetMemberId == likesParams.MemberId).Select(l => l.SourceMember);
                break;
            default: // "mutual"
